...
def upgrade() -> None:
    ...
    if 'user_profiles' not in existing_tables:
        ...
        logger.info("⏭️ user_profiles table already exists, skipping")
    ...
    if 'articles' not in existing_tables:
        ...
        logger.info("⏭️ articles table already exists, skipping")
    ...
    if 'rss_sources' not in existing_tables:
        ...
        logger.info("✅ rss_sources table created")
    else:
        logger.info("⏭️ rss_sources table already exists, skipping")

    # Create article_insights table
    if 'article_insights' not in existing_tables:
        ...
        logger.info("✅ article_insights table created")
    else:
        logger.info("⏭️ article_insights table already exists, skipping")
    ...
    if 'reports' not in existing_tables:
        ...
        logger.info("⏭️ reports table already exists, skipping")

    # Create user_interactions table
    if 'user_interactions' not in existing_tables:
        ...
        logger.info("✅ user_interactions table created")
    else:
        logger.info("⏭️ user_interactions table already exists, skipping")
...
def downgrade() -> None:
    ...
    inspector = sa.inspect(conn)
    existing_tables = inspector.get_table_names()
    logger.info(f"📊 Tables before rollback: {existing_tables}")
